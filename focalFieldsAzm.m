...
function [xi, yi, Exi, Eyi, Ezi] = focalFieldsAzm(lambda, NA, z, f0, f, rhoMax, N)
k0=2*pi/lambda;
...
tic
for rhoi = rho
    Iazm(i) = trapz(fw.*cos(theta).^(1/2).*(sin(theta).^2).*besselj(1,k0*rhoi*sin(theta)).*exp(1j*k0*z.*cos(theta)));
    j=1;
    for phii = phi
        Ex(i,j) = 1j*k0*f^2/2*sqrt(n1/n2)*E0*exp(-1j*k0*f)*(1j*Iazm(i)*sin(phii));
        Ey(i,j) = 1j*k0*f^2/2*sqrt(n1/n2)*E0*exp(-1j*k0*f)*(-1j*Iazm(i)*cos(phii));
        Ez(i,j) = 0;
        j=j+1;
    end
    i=i+1;
